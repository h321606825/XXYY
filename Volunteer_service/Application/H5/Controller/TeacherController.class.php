...
class TeacherController extends CommonController
{
	...
	public function wish_pool(){
		...
		if(IS_AJAX){
            ...
            $rows1 = M("wish")
	            ...
	            ->order("wish.id desc")
	            ->select();
	        foreach ($rows1 as $i => $value) {
	        	# code...
	        	if ($value['level'] == 8 && $value['shenhe'] != 1 && $value['status_p'] == 1) {
	        		# code...
	        		unset($rows1[$i]);
	        		$count1--;
	        	}
	...
	public function finish(){
		...
		$where['wish.id'] = $id;
		$where['wish.t_id'] = $tid;
		$result = M("wish")->where($where)->select();
		if(empty($result))
		{
	...
	public function claim(){
		...
		{
			$flag = 1;//学生表
			$student = M("student")->where("mobile = %s",$s_mobile)->find();
		}
	...
	public function wish_release(){
		...
		if(IS_AJAX){
	        ...
	        foreach ($rows as $i => $value) {
	        	# code...
	        	// var_dump($value);
	        	if(empty($value['t_name']))
	        	{
	        		$where['wish.id'] = array("eq",$rows[$i]['id']);
	        		// var_dump($rows[$i]['id']);
	        		// unset($rows[$i]);
	        		$rows2 = M("wish")
            			->field("wish.*,t.name AS t_name,t.mobile AS t_mobile,wu.headimgurl AS wu_headimgurl")
